...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.self_attention(x) # [batch_size, seq_len, hidden]
        x = self.att_layer_norm(x) # [batch_size, seq_len, hidden]
        x = self.dropout(self.fcn(x)) + x # [batch_size, seq_len, hidden]
        x = self.fcn_layer_norm(x)  # [batch_size, seq_len, hidden]
        # YOUR CODE ENDS HERE
        return x
...
class TransformerEncoder(nn.Module):
    def __init__(self, num_layers, hidden, num_heads, fcn_hidden, vocab_size, max_seq_len, dropout=0.1, causal=False):
        ...
        self.encoder_layers = nn.ModuleList([TransformerEncoderLayer(hidden, num_heads, fcn_hidden, dropout, causal) for i in range(num_layers)])
        # YOUR CODE ENDS HERE
    ...
    def _add_positions(self, sequence_tensor):
        ...
        positions = self.positional_emb(positions).unsqueeze(0).expand(sequence_tensor.size(0), seq_len, sequence_tensor.size(2))
        output = seq_len + positions
        # YOUR CODE ENDS HERE
    ...
    def forward(self, input_ids=None):
        ...
        # NOTE: Please write shape of the tensor for each line of code
        # YOUR CODE STARTS HERE(our implementation is about 4 lines)
        embeddings = self.encoder_emb(input_ids) # [batch_size, vocab_size, hidden]
        output = self._add_positions(embeddings) # [batch_size, vocab_size, hidden]
        for i in range(self.num_layers):
            output = self.encoder_layers[i](output) # [batch_size, vocab_size, hidden]
        # YOUR CODE ENDS HERE
        return output
...
class TransformerLM(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        logits = self.dropout(self.encoder(input_ids))
